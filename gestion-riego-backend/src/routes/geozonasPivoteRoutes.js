...
// Obtener todas las geozonas de un regador (filtradas por lote actual si está en uno)
router.get('/regador/:regadorId', verifyToken, async (req, res) => {
    const client = await pool.connect();
    try {
        const { regadorId } = req.params;

        // Primero, obtener la posición actual del regador para saber en qué lote está
        const queryPosicionActual = `
            SELECT 
                dog.geozona_id,
                gp.lote_id
        ...
        // ⚠️ CAMBIO: Solo filtrar por lote si está en uno Y tiene datos GPS recientes
        // Si no está en ninguna geozona, mostrar TODOS los sectores
        if (loteActual) {
            query += ` AND gp.lote_id = $2`;
            params.push(loteActual);
            console.log(`📍 Regador ${regadorId} está en lote ${loteActual}, filtrando sectores...`);
        } else {
            console.log(`📍 Regador ${regadorId} no está en ninguna geozona, mostrando todos los sectores`);
        }
