...
class VueltasRiegoService {
    ...
    async verificarCompletarVuelta(regadorId, anguloActual, timestamp) {
        try {
            ...
            if (duracionMinutos < this.DURACION_MINIMA_MINUTOS) {
                console.log(`?? Vuelta ${vueltaActiva.numero_vuelta} - Tiempo insuficiente: ${duracionMinutos} min (mínimo: ${this.DURACION_MINIMA_MINUTOS})`);
    ...
    async registrarSalidaSector(regadorId, geozonaId, timestamp) {
        try {
            ...
            // ðŸ”’ VERIFICAR SI DEBE MARCARSE COMO COMPLETADO
            // Contar reportes con presiÃ³n para determinar si el sector fue regado correctamente
            ...
            // Marcar como completado si al menos el 60% de los reportes tienen presiÃ³n
